...
#' @title Extrai informações sobre os mandato de todos os senadores
#' @description Recebeum dataframe contendo id e casa dos parlamentares e retorna um dataframe contendo informações do mandato, como legislatura,
#' data de inicio, data de fim, situacao, código e descrição da causa do fim do exercício.
#' @param df_parlamentares dataframe com informações de id e casa dos parlamentares
#' @return Dataframe contendo informações de legislatura,
#' data de inicio, data de fim, situacao, código e descrição da causa do fim do exercício.
#' @examples
#' extract_mandatos_senadores(readr::read_csv(here::here("crawler/raw_data/parlamentares.csv")))
extract_mandatos_senadores <- function(df_parlamentares = readr::read_csv(here::here("crawler/raw_data/parlamentares.csv"))) {
  library(tidyverse)
  
  senadores <- df_parlamentares %>% filter(casa == "senado")
  
  mandatos <-
    purrr::map2_df(senadores$id, senadores$casa, 
                   ~ extract_mandatos(.x, .y)) %>% 
    distinct()
  return(mandatos)
}